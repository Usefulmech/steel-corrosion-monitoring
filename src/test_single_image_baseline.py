"""
BASELINE: Single Image Tester for Cascaded YOLO Corrosion Pipeline.
Runs on raw PyTorch (.pt) files in full FP32 precision to prevent bounding box drift.
"""
...
# Pointing strictly to your raw PyTorch models
DETECTOR_PATH = BASE_DIR / "models" / "corrosion_detector.pt"
GRADER_PATH   = BASE_DIR / "models" / "corrosion_grader.pt"

# Ensure you have a picture named test_image.jpg in your main project folder
...
def main():
    print(f"Loading Raw PyTorch Models (High Precision)...")
    
    # Check if models exist before loading
    if not DETECTOR_PATH.exists() or not GRADER_PATH.exists():
        print(f"Error: Model files not found! Check if {DETECTOR_PATH.name} and {GRADER_PATH.name} are in the models folder.")
        return
    ...
    output_path = OUTPUT_DIR / f"baseline_analyzed_{TEST_IMAGE_PATH.name}"
    cv2.imwrite(str(output_path), annotated_img)
    print(f"\nHigh-precision annotated image saved to: {output_path}")

    cv2.imshow("PyTorch Baseline Image Analysis", annotated_img)
